...
instance Eq Board where
        Board cs fd fc == Board cs' fd' fc' = (fd == fd') && 
                                              (fc == fc') && 
...
data GameState = GameState { gameBoard :: Board, 
                                                 sourceMove :: Move } deriving (Show, Eq)
...
instance Show Move where
        show (Move (Card rk st) l1 l2) = show rk ++ " " ++ 
                                                                         show st ++ ": " ++ 
                                                                         show l1 ++ " -> " ++ 
                                                                         show l2 ++ "\n"
        show BeginGame = ""
...
playableCascades (Board stacks _ _) cd = findIndices playableCascade stacks
        where
                ...
                playableCascade (st:_) = (black cd == red st) && 
...
allCardPlaysNoFC bd card source = pf ++ stackplays
        where
                pf = [GameState (pushFoundation bd card) (Move card source Foundations) 
...
allPermissable bd = filter (\y->gameBoard y /= bd) $ 
        ...
        where
                ...
                sources = replicate (length fccards) FreeCells ++ 
